...
(S) Let me tell you a time when I went above and beyond my job responsibilities. When I was working on my current project, sometimes my manager needs to delete container, model or firmware. There are a lot of dependencies and anytime synchronization may not be done properly. (T) So I thought to make a feature for my manager to do this task easily. (A) I developed a feature for testing for my manager as well as others (Like PDM, testers) that calling an API with secretToken will delete all dependencies for model, container and firmware. (R) This feature were really amazing when anyone faced problem with synchronization. Also then my manager decided to keep this feature as hidden feature of my project!
